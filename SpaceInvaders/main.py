	...
	while run:
		...
		if len(enemies) == 0:
			...
			for i in range(wave_length):
				enemy = Enemy(random.randrange(100, w - 100), random.randrange(-1500, -100), random.choice(["red", "green", "blue"]))
		...
		if keys[pygame.K_a] and player.x - player_vel > 0 or keys[pygame.K_LEFT] and player.x - player_vel > 0: # Left
			player.x -= player_vel
		if keys[pygame.K_d] and player.x + player_vel + player.get_width() < w or keys[pygame.K_RIGHT] and player.x + player_vel + player.get_width() < w: # Right
			player.x += player_vel
		if keys[pygame.K_w] and player.y - player_vel > 0 or keys[pygame.K_UP] and player.y - player_vel > 0: # Up
			player.y -= player_vel
		if keys[pygame.K_s] and player.y + player_vel + player.get_height() + 15 < h or keys[pygame.K_DOWN] and player.y + player_vel + player.get_height() + 15 < h: # Down
			player.y += player_vel
...
def main_menu():
	...
	while run:
		win.blit(BG, (0, 0))
		title_label = title_font.render("Click to Play!", 1, (255, 255, 255))
